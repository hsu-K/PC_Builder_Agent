...
def _build_ecommerce_system_prompt(db_path: str) -> str:
    ...
    return (
        ...
        "FULL BUILD / 完整菜單 / 配一台電腦 / 遊戲機 / 主機 / 組電腦:\n"
        "- When the user wants a COMPLETE build with a budget, call "
        "recommend_pc_build_tool(budget=<整數>, use_case=...) FIRST instead of querying "
        "each category one by one. use_case: 'gaming'(預設) / '4k_gaming' / 'office'(文書)。\n"
...
def ecommerce_node(
    ...
    *,
    ...
    Returns:
        dict: ``{"messages": [ai_message], "ecommerce_advice": text}``
    """

    # db_path 來源:state.ecommerce_db_path 優先,否則用預設正式資料庫。
    # 由於 run_agent_turn 沒有針對 ecommerce 工具自動注入參數的機制,
    # 這裡改用 prompt 明確引導 LLM 在每次工具呼叫都帶上正確的 db_path
    # (符合 Phase 3 規格「在 prompt 或工具呼叫引導中使用該 DB path」)。
