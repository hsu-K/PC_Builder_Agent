...
1. 使用者透過 CLI 或 chat 介面發出需求（包含 `preferences`：例如 `budget`, `use_case`）。
2. Router 決定需要啟動哪些 node（例如：若需要最新討論則啟動 `pc_board_scraper` 爬蟲 node）。
3. Node 呼叫 `run_agent_turn()` 與 LLM 互動，並可選擇呼叫 `tools` 提供的功能（例如 `pc_board_scraper.invoke()`）。
4. Node 處理結果並回寫到 `state`。
5. 最終由整合 node 或回應流程將結果呈現給使用者。
